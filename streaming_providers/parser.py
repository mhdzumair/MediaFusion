...
SEASON_EPISODE_PATTERNS: dict[str, tuple[Pattern, Callable[[re.Match, int], tuple[int, int]]]] = {
    ...
    "season_ep": (
        re.compile(r"(?:[sS]eason|[sS]eries)[.\s-]*(\d{1,2}).*?(?:[eE]p|[eE]pisode)?[.\s-]*(\d{1,2})"),
        lambda m, _: (int(m.group(1)), int(m.group(2))),
    ),
...
ANIME_PATTERNS: dict[str, tuple[Pattern, Callable[[re.Match, int], tuple[int, int]]]] = {
...
class TorrentFileProcessor:
    ...
    def _process_files(self) -> list[FileInfo]:
        ...
        for idx, file in enumerate(self.files):
            file_info = FileInfo.from_torrent_file(idx, file, self.name_key, self.size_key)
    ...
    def get_largest_video_file(self) -> FileInfo | None:
    ...
    async def _parse_season_episode_info(
        ...
        if not parsed_data.get("date"):
            ...
            if date_str_match:
                parsed_data["date"] = dateparser.parse(date_str_match.group(0)).strftime("%Y-%m-%d")
        ...
        season, episode = fallback_parse_season_episode(file_info.filename, default_season)

        # Log when fallback parser is used
        if season is not None or episode is not None:
            logger.info(f"Used fallback parser for file '{file_info.filename}'. Detected: S{season}E{episode}")
    ...
    async def parse_all_episodes(
        self, torrent_title: str, meta_id: str, default_season: int | None = None
    ) -> list[StreamFileData]:
        """Parse all episode information from video files.

        Returns StreamFileData objects with embedded episode info for series torrents.
        """
        episodes = []
        for file_info in self.get_video_files():
            season, episode = await self._parse_season_episode_info(file_info, torrent_title, meta_id, default_season)
    ...
    def find_file_by_name(self, filename: str) -> FileInfo | None:
...
async def select_file_index_from_torrent(
    ...
    name_key: str = "name",
    size_key: str = "size",
    file_size_callback: Callable | None = None,
    ...
    if season and episode:
        selected_file = await processor.find_specific_episode(
            season, episode, torrent_stream.name, torrent_stream.meta_id
        )
        if selected_file:
            ...
            raise ProviderException(
                "Found video files but couldn't match season/episode. Annotation has been requested.",
...
async def _save_torrent_stream(torrent_stream: TorrentStreamData) -> None:
    ...
    v5 schema: File structure is stored in StreamFile table, not on the stream directly.
    """
    updates = {
        "is_blocked": torrent_stream.is_blocked,
        "total_size": torrent_stream.size,  # TorrentStream uses total_size, not size
    ...
    info_hash = torrent_stream.info_hash
    ...
    await run_db_operation_with_retry(
        _persist,
        operation_name=f"save torrent stream {info_hash}",
        max_attempts=4,
        initial_delay_seconds=0.75,
        on_retry=lambda attempt, max_a, exc: logger.warning(
            "Retry %d/%d saving torrent stream %s: %s",
            attempt,
            max_a,
            info_hash,
            exc,
...
async def update_torrent_streams_metadata(
    ...
    v5 schema: File info is stored in StreamFile table with embedded episode data.
    """
    # Check if annotation was recently requested with redis lock for 3 days
    acquired, _ = await acquire_redis_lock(f"annotation_lock_{torrent_stream.info_hash}", timeout=259200, block=False)
    if not acquired:
        logger.info(f"Skipping metadata update for {torrent_stream.info_hash} - recent annotation request")
        return False

    try:
        if season is None:
            # Movie handling - get the largest video file
            file_info = processor.get_largest_video_file()
            if not file_info:
                torrent_stream.is_blocked = True
                await _save_torrent_stream(torrent_stream)
                logger.error(f"No video files found in torrent {torrent_stream.info_hash}")
                return False

            # Create file entry for movie (v5 schema)
            torrent_stream.files = [
                StreamFileData(
                    file_index=file_info.index if is_index_trustable else 0,
                    filename=file_info.filename,
                    size=file_info.size,
                    file_type="video",
                )
            ...
            # Parse episodes - returns StreamFileData with embedded episode info
            ...
            if not episodes:
                # Try to use existing file info if single file
                if len(video_files) == 1 and len(torrent_stream.files) == 1:
                    existing_file = torrent_stream.files[0]
                    # Update the existing file with parsed info
                    existing_file.filename = video_files[0].filename
                    existing_file.size = video_files[0].size
                    if is_index_trustable:
                        existing_file.file_index = video_files[0].index
                    episodes = [existing_file]
                else:
                    await _request_annotation(torrent_stream, processor.file_infos)
                    return False

            torrent_stream.files = episodes
            total_size = sum(f.size for f in processor.get_video_files())
            if total_size > torrent_stream.size:
                torrent_stream.size = total_size

        torrent_stream.updated_at = datetime.now(tz=UTC)
        await _save_torrent_stream(torrent_stream)
        logger.info(f"Successfully updated {torrent_stream.info_hash} metadata")
        return True

    except Exception as e:
        logger.error(
            f"Error updating metadata for {torrent_stream.info_hash}: {str(e)}",
            exc_info=True,
        )
        return False


async def _request_annotation(torrent_stream: TorrentStreamData, file_infos: list[FileInfo]):
    """Request manual annotation for the torrent stream.

    v5 schema: File details are stored in StreamFile table, not as JSON.
    We create StreamFile entries without episode linking, awaiting manual annotation.
    """
    # Create StreamFile entries for all files (without episode linking for now)
    torrent_stream.files = [
        StreamFileData(
            file_index=f.index,
            filename=f.filename,
            size=f.size,
            file_type="video" if is_video_file(f.filename) else "other",
        )
        for f in file_infos
    ]
    await _save_torrent_stream(torrent_stream)

    # Notify contributors (e.g. Telegram bot - registered at app startup)
    await send_file_annotation_request(torrent_stream.info_hash, torrent_stream.name)
    logger.info(f"Requested annotation for {torrent_stream.info_hash}")
...
def is_likely_hash(match_str: str, filename: str) -> bool:
    ...
    bracket_match = re.search(r"\[[^\]]*" + re.escape(match_str) + r"[^\[]*\]", filename)
    if bracket_match:
...
def fallback_parse_season_episode(filename: str, default_season: int = 1) -> tuple[int | None, int | None]:
    """
    ...
    for pattern_name, (regex, extractor) in SEASON_EPISODE_PATTERNS.items():
        ...
        if match:
            ...
            if pattern_name in ["simple_episode"] and is_likely_hash(match.group(0), base_filename):
                continue
...
# =========================================================================
# Usenet/NZB File Selection Functions
# =========================================================================


async def select_file_index_from_usenet_stream(
    ...
    """
    Select the file index from Usenet download info.
    Similar to select_file_index_from_torrent but for Usenet content.

    Args:
        usenet_info: Dictionary containing file information from Usenet provider
        usenet_stream: UsenetStreamData object
        filename: Target filename to match
        season: Season number for series
        episode: Episode number for series
        file_key: Key to access files list in usenet_info
        name_key: Key to access filename in each file dict
        size_key: Key to access file size in each file dict
        is_filename_trustable: Whether the filename data is trustworthy
        is_index_trustable: Whether the file index is trustworthy

    Returns:
        Index of the selected file
    """
    files = usenet_info.get(file_key, [])
    ...
    if filename:
        for idx, file in enumerate(files):
            file_name = basename(file.get(name_key, ""))
            if file_name == filename:
                return idx

    # Initialize processor for further processing
    processor = UsenetFileProcessor(usenet_info, file_key, name_key, size_key)

    # Check if there are any video files
    video_files = processor.get_video_files()
    if not video_files:
        raise ProviderException(
            "No valid video files found in Usenet download",
            "no_matching_file.mp4",
        )
    ...
    if season and episode:
        selected_file = await processor.find_specific_episode(
            season, episode, usenet_stream.name, usenet_stream.meta_id
        )
        if selected_file:
            return selected_file.index
    ...
    raise ProviderException(
        "No valid video file found in Usenet download",
        "no_matching_file.mp4",
    )


class UsenetFileProcessor:
    """Processor for Usenet download files, similar to TorrentFileProcessor."""
    ...
    def __init__(
        ...
        self.usenet_info = usenet_info
        self.file_key = file_key
        self.name_key = name_key
        self.size_key = size_key
        self.files = usenet_info.get(file_key, [])
        self.file_infos = self._process_files()
        self._video_files: list[FileInfo] | None = None
        self._metadata = None
    ...
    def get_video_files(self) -> list[FileInfo]:
        """Get all video files, caching the result."""
        if self._video_files is None:
            self._video_files = [f for f in self.file_infos if f.is_video]
        return self._video_files
    ...
    async def find_specific_episode(self, season: int, episode: int, title: str, meta_id: str) -> FileInfo | None:
        """Find a specific episode in the video files."""
        video_files = self.get_video_files()

        for file_info in video_files:
            parsed_season, parsed_episode = await self._parse_season_episode_info(
                file_info, title, meta_id, default_season=season
            )
            if parsed_season == season and parsed_episode == episode:
                return file_info

        # If only one video file, return it
    ...
    async def _parse_season_episode_info(
        ...
        # Try from title
        title_parsed = PTT.parse_title(title)
        title_seasons = title_parsed.get("seasons", [])
        title_episodes = title_parsed.get("episodes", [])
        ...
        # Check for date-based episode identification
        if not parsed_data.get("date"):
            date_str_match = DATE_STR_REGEX.search(file_info.filename)
            if date_str_match:
                parsed_data["date"] = dateparser.parse(date_str_match.group(0)).strftime("%Y-%m-%d")
