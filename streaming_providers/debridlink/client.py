...
class DebridLink(DebridClient):
    ...
    async def _make_request(
        ...
        **kwargs,
        ...
        return await super()._make_request(method=method, url=url, data=data, json=json, params=params, **kwargs)

    @staticmethod
    def _handle_error_message(error_message):
        match error_message:
            case "freeServerOverload":
                raise ProviderException("Debrid-Link free servers are overloaded", "need_premium.mp4")
            case "badToken" | "expired_token":
                raise ProviderException("Invalid token", "invalid_token.mp4")
            case "server_error" | "notDebrid":
                raise ProviderException("Debrid-Link server error", "debrid_service_down_error.mp4")
            case "maxLink" | "maxLinkHost" | "maxData" | "maxDataHost" | "maxTorrent":
                raise ProviderException("Debrid-Link daily limit reached", "daily_download_limit.mp4")
            ...
            case "floodDetected":
                raise ProviderException("Debrid-Link flood detected", "too_many_requests.mp4")
    ...
    async def initialize_headers(self):
        if self.token:
            ...
            if token_code:
                access_token_data = await self.refresh_token(self.OPENSOURCE_CLIENT_ID, token_code)
    ...
    async def get_torrent_files_list(self, torrent_id) -> dict[str, Any]:
        return await self._make_request("GET", f"{self.BASE_URL}/files/{torrent_id}/list")

    async def delete_torrent(self, torrent_id) -> dict[str, Any]:
        return await self._make_request("DELETE", f"{self.BASE_URL}/seedbox/{torrent_id}/delete")

    async def disable_access_token(self) -> dict[str, Any] | None:
        return await self._make_request(
            "GET",
