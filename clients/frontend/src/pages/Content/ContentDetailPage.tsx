...
export function ContentDetailPage() {
  ...
  const communityStreamIds = useMemo(() => {
    const ids = new Set(paginatedStreamIds)
    if (selectedStream?.id) ids.add(selectedStream.id)
    return [...ids]
  }, [paginatedStreamIds, selectedStream?.id])
