...
import { useContributions, useContributionStats, useDeleteContribution, useMyStreamSuggestions, useDeleteStreamSuggestion } from '@/hooks'
...
function parseContributionData(data: Record<string, unknown>, type: ContributionType): { label: string; value: string; type: 'text' | 'link' | 'badge' | 'size' | 'code' }[] {
...
function ContributionDetailsContent({ contribution }: { contribution: { id: string; contribution_type: ContributionType; status: ContributionStatus; target_id?: string; data: Record<string, unknown>; created_at: string; reviewed_at?: string; review_notes?: string } }) {
  ...
  return (
    <div className="space-y-5 py-2">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <Badge 
          variant="secondary" 
          className={`${statusConfig[contribution.status]?.color ?? ''} bg-opacity-10`}
        >
          {(() => {
      ...
      {displayName && (
        <div className="p-4 rounded-md hero-gradient border border-primary/20">
          ...
          <p className="font-medium text-lg break-all font-display">
            {displayName}
          </p>
        </div>
      )}
      ...
      {parsedFields.length > 0 && (
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {parsedFields.filter(f => f.label !== 'Torrent Name' && f.label !== 'Title').map((field, idx) => (
              <div 
                key={idx} 
                className={`p-3 rounded-md bg-muted/40 border border-border/50 ${
                  field.type === 'code' || (field.type === 'text' && String(field.value).length > 40) ? 'col-span-2' : ''
                }`}
              >
        ...
        <div className="space-y-2">
          ...
          <div className="p-3 rounded-md bg-muted/40 border border-border/50">
            <div className="flex items-start gap-2">
              <code className="text-xs font-mono break-all line-clamp-3 flex-1">
                {magnetLink}
              </code>
              <Button
                variant="ghost"
      ...
      <div className="flex items-center justify-between text-xs text-muted-foreground pt-2 border-t border-border/50">
        ...
        {contribution.reviewed_at && (
          <span>
            Reviewed: {new Date(contribution.reviewed_at).toLocaleString()}
          </span>
        )}
      </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-1">
          Track the status of your metadata and stream corrections
        </p>
      </div>
          ...
          {activeTab === 'streams' && (
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => setStreamStatusFilter(undefined)}>
                  All Status
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStreamStatusFilter('pending')}>
                  Pending
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStreamStatusFilter('approved')}>
                  Approved
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStreamStatusFilter('auto_approved')}>
                  Auto-Approved
                </DropdownMenuItem>
                <DropdownMenuItem onClick={() => setStreamStatusFilter('rejected')}>
                  Rejected
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  ...
                  <DropdownMenuItem onClick={() => setTypeFilter('metadata')}>
                    Metadata Fix
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setTypeFilter('stream')}>
                    New Stream
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setTypeFilter('torrent')}>
                    Torrent Imports
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
        ...
        <TabsContent value="streams" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Corrections to stream metadata (quality, language, episode info, etc.)
              </CardDescription>
            </CardHeader>
            <CardContent>
              {streamLoading ? (
                ...
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">
                    Browse content and click on streams to suggest corrections.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {streamSuggestions.suggestions.map((suggestion) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0 space-y-1">
                          ...
                          {suggestion.field_name && (suggestion.current_value || suggestion.suggested_value) && (
                            <div className="flex items-center gap-2 text-sm">
                              <span className="text-red-400 line-through truncate max-w-[150px]" title={suggestion.current_value || ''}>
                        ...
                        <Badge 
                          variant="secondary" 
                          className={`${status?.color} bg-opacity-10 flex-shrink-0`}
                        >
        ...
        <TabsContent value="metadata" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Metadata corrections and torrent imports
              </CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
      ...
      <Dialog open={!!detailsDialogOpen} onOpenChange={() => setDetailsDialogOpen(null)}>
        <DialogContent className="sm:max-w-[700px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {selectedContribution && (() => {
                ...
                const typeColor = typeConfig[selectedContribution.contribution_type]?.color ?? 'text-primary bg-primary/10'
                return (
          ...
          {selectedContribution && (
            <ContributionDetailsContent contribution={selectedContribution} />
          )}
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4 py-4">
              <div className="flex items-center gap-4 flex-wrap">
                ...
                <Badge variant="outline">
                  {formatSuggestionType(streamDetailsOpen.suggestion_type)}
                </Badge>
                {streamDetailsOpen.field_name && (
                  <Badge variant="outline">
                    {formatFieldName(streamDetailsOpen.field_name)}
                  </Badge>
                )}
                  ...
                  <div className="grid grid-cols-[1fr_auto_1fr] gap-2 items-start">
                    <div>
                      ...
                      <p className="text-sm text-red-400 break-all">
                        {streamDetailsOpen.current_value || '(empty)'}
                      </p>
                    </div>
              ...
              <div className="flex justify-between text-sm text-muted-foreground">
                <span>
                  Submitted: {new Date(streamDetailsOpen.created_at).toLocaleString()}
                </span>
                {streamDetailsOpen.reviewed_at && (
                  <span>
                    Reviewed: {new Date(streamDetailsOpen.reviewed_at).toLocaleString()}
                  </span>
                )}
              </div>
