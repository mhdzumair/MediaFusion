...
function parseFilenameForSearch(filename: string): string {
  ...
  const simplified = cleaned
    ...
    // Quality/Source
    .replace(
      /\b(bluray|blu-ray|bdrip|brrip|webrip|web-dl|webdl|web|hdtv|hdrip|dvdrip|dvdscr|dvd|hdcam|cam|ts|telesync|r5|ppvrip|hdrip|pdtv|dsr|tvrip|satrip|remux)\b/gi,
      '',
    )
    // Video codecs
    ...
    // Audio codecs
    .replace(
      /\b(aac|ac3|dts|dts-hd|dts-ma|truehd|atmos|flac|mp3|eac3|dd5\.?1|dd7\.?1|7\.1|5\.1|2\.0|stereo|mono|dolby|digital)\b/gi,
      '',
    )
    // HDR formats
    ...
    .replace(
      /\b(yts|yify|rarbg|sparks|geckos|ntg|flux|tigole|qxr|psa|joy|fgt|ethd|mkvking|mkvcage|pahe|evo|cmrg|stuttershit|amzn|nf|dsnp|hmax|atvp|pcok|hulu|criterion|mubi)\b/gi,
      '',
    )
    // Scene tags
    .replace(
      /\b(proper|repack|rerip|real|internal|limited|extended|unrated|directors\s*cut|theatrical|imax|3d|sbs|hou|dubbed|dual\s*audio|multi|subbed|hardcoded|hc)\b/gi,
      '',
    )
  ...
  if (value?.id) {
    return (
      <div className="flex items-center gap-2 p-2 rounded-lg border border-primary/30 bg-primary/5">
        ...
        {value.poster ? (
          <img src={value.poster} alt="" className="w-10 h-14 rounded object-cover flex-shrink-0" />
        ) : (
  ...
  return (
    ...
    >
      <PopoverTrigger asChild>
        <Button variant="outline" className="w-full justify-start text-muted-foreground h-12" disabled={disabled}>
            ...
            <ScrollArea className="max-h-[300px]">
              ...
              {!isLoading && !isFetching && debouncedQuery.length >= 2 && searchResults.length === 0 && (
                <div className="py-6 text-center">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setShowManualId(true)}>
                    <Link2 className="h-3 w-3 mr-1.5" />
                    Enter ID manually
                  </Button>
                </div>
              ...
              {searchResults.length > 0 && (
                <div className="p-2">
                  ...
                  {searchResults.map((result, idx) => (
                    ...
                    >
                      {result.poster ? (
                        <img src={result.poster} alt="" className="w-10 h-14 rounded object-cover flex-shrink-0" />
                      ) : (
                        <div className="w-10 h-14 rounded bg-muted flex items-center justify-center flex-shrink-0">
                          {metaType === 'series' ? (
                            <Tv className="h-4 w-4 text-muted-foreground" />
    ...
    >
      <CardHeader className="p-4 pb-2">
        <div className="flex items-start gap-3">
          <div className={cn('p-2 rounded-lg flex-shrink-0', isLinked ? 'bg-green-500/20' : 'bg-muted')}>
            <FileVideo className={cn('h-5 w-5', isLinked ? 'text-green-600' : 'text-muted-foreground')} />
          </div>
          <div className="flex-1 min-w-0">
            <CardTitle className="text-sm font-medium truncate">{getFilenameOnly(file.filename)}</CardTitle>
...
export function MultiContentWizard({
  ...
  const handleMetadataSelect = useCallback(
    (index: number, result: CombinedSearchResult) => {
      setFiles((prev) =>
        prev.map((f, i) => {
          if (i !== index) return f
          return {
  ...
  // Count linked files
  const linkedCount = useMemo(() => files.filter((f) => f.meta_id).length, [files])
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Link each file to its metadata</p>
            </div>
          </div>
              ...
              <div className="space-y-2 pr-4">
                {files.map((file, index) => (
                  <div key={index} className="flex items-center gap-3 p-3 rounded-lg border bg-card">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{getFilenameOnly(file.filename)}</p>
              ...
              <div className="space-y-3 pr-4">
                {files
                  ...
                  .map((file) => {
                    ...
                    return (
                      <div key={file.filename} className="flex items-center gap-4 p-4 rounded-lg border bg-card group">
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-yellow-700">
                          {files.filter((f) => !f.meta_id).length} file
                          {files.filter((f) => !f.meta_id).length !== 1 ? 's' : ''} not linked
                        </p>
      ...
      <div className="flex-shrink-0 border-t p-4">
        <div className="flex items-center justify-between">
          <Button variant="outline" onClick={currentStep === 'files' ? onCancel : handleBack} disabled={isImporting}>
