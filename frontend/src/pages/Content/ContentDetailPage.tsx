...
import { useCatalogItem, useCatalogStreams, useLibraryCheck, useAddToLibrary, useRemoveFromLibraryByMediaId, useTrackStreamAction, useCreateStreamSuggestion, useProfiles, useDeleteEpisodeAdmin, useUpdateWatchProgress, type CatalogType } from '@/hooks'
...
import { StreamVoteButtons, StreamReport, StreamCard, StreamFilters, StreamGroupedList, ViewModeToggle, StreamEditSheet, FileAnnotationDialog, StreamRelinkButton, CollectionAnnotationDialog, defaultStreamFilters, type StreamFilterState, type ViewMode, type FileLink, type EditedFileLink } from '@/components/stream'
import { MetadataActions, MetadataEditSheet, ContentLikesBadge, RefreshMetadataButton, ScrapeContentButton, ExternalIdsDisplay, BlockContentButton } from '@/components/metadata'
...
function StreamActionDialog({
  ...
  const handleOpenFileAnnotation = async () => {
    ...
    try {
      ...
      if (stream.episode_links && stream.episode_links.length > 0) {
        setFileLinks(stream.episode_links.map(el => ({
          file_id: el.file_id,
  ...
  return (
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent 
        className="glass border-border/50 sm:max-w-[500px]"
        onOpenAutoFocus={(e) => e.preventDefault()}
      >
        <DialogHeader>
          <DialogTitle>Stream Actions</DialogTitle>
          <DialogDescription>
            Choose what to do with this stream
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {stream.resolution && (
                  <Badge variant="outline">{stream.resolution}</Badge>
                )}
                {stream.size && (
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              This will block the stream from appearing in search results. The stream can be unblocked later from the admin panel.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                ...
                <>
                  ...
                  {hasMediaflowProxy || isDirectProxyStream ? (
                    ...
                    <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm">
                      <p className="text-amber-600 dark:text-amber-400 font-medium mb-1">Web Browser Playback Disabled</p>
                      <p className="text-muted-foreground text-xs">
                        To play streams in your browser, go to Configure → External Services → MediaFlow and enable "Enable Web Browser Playback". 
                        MediaFlow proxy is required for browser playback due to CORS restrictions.
                      </p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        ...
                        const textToCopy = isTorrentStream && stream.info_hash 
                          ? stream.info_hash 
                          : streamUrl
...
export function ContentDetailPage() {
  ...
  useEffect(() => {
    if (profiles && profiles.length > 0 && selectedProfileId === undefined) {
      ...
      const primaryService = defaultProfile.streaming_providers?.primary_service
  ...
  const filteredStreams = useMemo(() => {
    ...
    const { qualityFilter, resolutionFilter, sourceFilter, codecFilter, cachedFilter, streamTypeFilter, sortBy, sortOrder } = streamFilters
  ...
  const handleWatchStream = useCallback(async (stream: CatalogStreamInfo, streamUrl: string) => {
              ...
              <div className="flex flex-wrap gap-2">
                {item.genres.map(genre => (
                  <Link key={genre} to={`/dashboard/library?tab=browse&genre=${encodeURIComponent(genre)}`}>
                    <Badge variant="secondary" className="rounded-lg hover:bg-primary/20 hover:text-primary cursor-pointer transition-colors">
                      {genre}
                    </Badge>
                  </Link>
                ))}
              ...
              <div className="grid gap-3 text-sm max-w-2xl">
                {item.directors && item.directors.length > 0 && (
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    <span className="text-muted-foreground font-medium">Director{item.directors.length > 1 ? 's' : ''}:</span>
                    {item.directors.map((director, idx) => (
                      ...
                      >
                        {director}{idx < item.directors!.length - 1 ? ',' : ''}
                      </Link>
                    ))}
                  </div>
                )}
                {item.writers && item.writers.length > 0 && (
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    <span className="text-muted-foreground font-medium">Writer{item.writers.length > 1 ? 's' : ''}:</span>
                    {item.writers.slice(0, 5).map((writer, idx) => (
                      <Link 
                        ...
                        className="hover:text-primary transition-colors"
                      >
                        {writer}{idx < Math.min(item.writers!.length, 5) - 1 ? ',' : ''}
                      </Link>
                    ))}
                    {item.writers.length > 5 && <span className="text-muted-foreground">+{item.writers.length - 5} more</span>}
                  </div>
                )}
                  ...
                  <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                    ...
                    {item.cast.slice(0, 8).map((actor, idx) => (
                      <Link 
                        ...
                        className="hover:text-primary transition-colors"
                      >
                        {actor}{idx < Math.min(item.cast!.length, 8) - 1 ? ',' : ''}
                      </Link>
                    ))}
                    {item.cast.length > 8 && <span className="text-muted-foreground">+{item.cast.length - 8} more</span>}
                  </div>
                )}
              </div>
            )}
                ...
                {(item.last_refreshed_at || item.last_scraped_at) && (
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground mt-2">
                    {item.last_refreshed_at && (
                      <p>
                        Metadata refreshed: {new Date(item.last_refreshed_at).toLocaleDateString(undefined, { 
                          year: 'numeric', 
                          month: 'short', 
                          day: 'numeric',
                          hour: '2-digit',
                    ...
                    {item.last_scraped_at && (
                      <p>
                        Streams scraped: {new Date(item.last_scraped_at).toLocaleDateString(undefined, { 
                          year: 'numeric', 
                          month: 'short', 
                          day: 'numeric',
                          hour: '2-digit',
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {profiles && profiles.length > 1 ? (
                    ...
                    <Badge variant="outline" className="rounded-lg px-2.5 sm:px-3 py-1.5 h-9 flex items-center text-xs sm:text-sm">
                      {profiles[0].name}
                    </Badge>
                  ) : null}
