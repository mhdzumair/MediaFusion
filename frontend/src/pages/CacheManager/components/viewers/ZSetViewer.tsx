  ...
  const sortedAndFilteredData = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (entry) => entry.member.toLowerCase().includes(lowerSearch) || String(entry.score).includes(searchTerm),
      )
    }
            ...
            <TableHeader className="sticky top-0 bg-muted/80 backdrop-blur-sm z-10">
              <TableRow className="hover:bg-transparent">
                <TableHead className="w-[180px]">
                  ...
                  >
                    Score
                    <ArrowUpDown className={cn('h-3.5 w-3.5', sortField === 'score' && 'text-primary')} />
                  </Button>
                </TableHead>
                <TableHead>
                  <Button
                  ...
                  >
                    Member
                    <ArrowUpDown className={cn('h-3.5 w-3.5', sortField === 'member' && 'text-primary')} />
                  </Button>
                </TableHead>
                          ...
                          <TooltipProvider>
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span className="text-emerald-400 cursor-help">{scoreFormatted.display}</span>
                              </TooltipTrigger>
                              <TooltipContent>
