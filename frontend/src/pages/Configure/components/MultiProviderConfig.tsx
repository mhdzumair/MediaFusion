...
import { 
  Eye, EyeOff, ExternalLink, Loader2, Copy, CheckCircle2, AlertCircle,
  Plus, Trash2, ChevronUp, ChevronDown, Power, PowerOff
} from 'lucide-react'
...
import type { ConfigSectionProps, QBittorrentConfig, SABnzbdConfig, NZBGetConfig, EasynewsConfig, StreamingProviderConfigType } from './types'
...
function SingleProviderEditor({
  ...
  const updateQBConfig = (updates: Partial<QBittorrentConfig>) => {
    const currentQbc = provider.qbc || {
      qur: '', qus: '', qpw: '', stl: 1440, srl: 1.0, pva: 100, cat: 'MediaFusion',
      wur: '', wus: '', wpw: '', wdp: '/'
    }
  ...
  const updateSABnzbdConfig = (updates: Partial<SABnzbdConfig>) => {
    const currentSbc = provider.sbc || {
      u: '', ak: '', cat: 'MediaFusion', wur: '', wus: '', wpw: '', wdp: '/'
    }
  ...
  const updateNZBGetConfig = (updates: Partial<NZBGetConfig>) => {
    const currentNgc = provider.ngc || {
      u: '', un: '', pw: '', cat: 'MediaFusion', wur: '', wus: '', wpw: '', wdp: '/'
    }
  ...
  const verificationUrl = oauthDeviceCode?.direct_verification_url || 
                          oauthDeviceCode?.verification_url || 
    ...
    <div className={cn(
        ...
        <div className="flex-1 flex items-center gap-2 min-w-0">
          {selectedProvider ? (
            <>
              ...
              {provider.n && (
                <Badge variant="outline" className="text-xs font-mono">{provider.n}</Badge>
              )}
              <Badge variant="secondary" className="text-xs">{selectedProvider.type}</Badge>
            </>
          ) : (
      ...
      {isExpanded && (
        <div className="p-4 pt-0 space-y-4 border-t">
                ...
                {availableProviders.map((p) => (
                  <SelectItem key={p.value} value={p.value}>
                    <div className="flex items-center gap-2">
                      <span>{p.icon}</span>
                      <span>{p.label}</span>
                      <Badge variant="secondary" className="ml-auto text-xs">{p.type}</Badge>
                    </div>
                  </SelectItem>
                ))}
              </SelectContent>
              ...
              <AccordionItem value="sabnzbd">
                ...
                <AccordionContent className="space-y-4 pt-4">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <p className="text-xs text-muted-foreground">
                      Path to completed downloads folder on WebDAV server
                    </p>
                  </div>
                </AccordionContent>
              </AccordionItem>
          ...
          {selectedProvider && selectedProvider.value && selectedProvider.value !== 'p2p' && selectedProvider.value !== 'qbittorrent' && selectedProvider.value !== 'sabnzbd' && selectedProvider.value !== 'nzbget' && selectedProvider.value !== 'easynews' && (
            <div className="space-y-3 pt-2 border-t">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={provider.ewc !== false}
                  onCheckedChange={(checked) => onUpdate({ ewc: checked })}
                />
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={provider.oscs === true}
                  onCheckedChange={(checked) => onUpdate({ oscs: checked })}
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Paste the code on the website and click authorize. This page will automatically detect when you're done.
                </p>
              </div>
        ...
        {providers.length === 0 ? (
          <Alert>
            ...
            <AlertDescription>
              ...
              Without a provider, only P2P streaming will be available.
            </AlertDescription>
          </Alert>
        ) : (
