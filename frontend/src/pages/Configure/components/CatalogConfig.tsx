  ...
  const toggleCatalog = (catalogId: string) => {
    ...
    if (existingConfig) {
      ...
      if (existingConfig.en !== false) {
        ...
        updateCatalogConfigs(catalogConfigs.map((c) => (c.ci === catalogId ? { ...c, en: true } : c)))
      }
    } else {
    ...
    if (existingConfig) {
      updateCatalogConfigs(
        catalogConfigs.map((c) =>
          c.ci === catalogId ? { ...c, s: sortValue as CatalogConfigType['s'], o: order } : c,
        ),
      )
    } else {
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            variant="ghost"
            size="icon"
            className={cn('h-7 w-7', hasCustomSort && 'text-primary')}
        ...
        <PopoverContent className="w-64" onClick={(e) => e.stopPropagation()}>
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Select value={currentSort} onValueChange={(value) => updateCatalogSort(catalogId, value, currentOrder)}>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">📚 Catalog Configuration</CardTitle>
            <CardDescription>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between pb-4 border-b">
          <div className="space-y-0.5">
            ...
            <p className="text-xs text-muted-foreground">Show catalog browsing in Stremio</p>
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant={showReorder ? 'default' : 'outline'} size="sm" onClick={() => setShowReorder(!showReorder)}>
