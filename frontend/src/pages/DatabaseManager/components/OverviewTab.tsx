...
function HealthIndicator({ value, label, good, warning }: {
  value: number
  label: string
  good: number
  warning: number
...
export function OverviewTab({ onTableClick }: OverviewTabProps) {
  const { data: stats, isLoading: statsLoading, refetch: refetchStats, isRefetching: statsRefetching } = useDatabaseStats()
  const { data: tables, isLoading: tablesLoading, refetch: refetchTables, isRefetching: tablesRefetching } = useTableList()
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-card/50 border-border/50">
          ...
          <CardContent className="space-y-4">
            ...
            />
            <HealthIndicator 
              value={100 - connectionUsage} 
              label="Connection Availability" 
              good={50} 
              warning={20} 
            />
            ...
            <div className="pt-4 border-t border-border/50 space-y-3">
              ...
              <div className="flex items-center justify-between text-sm">
                ...
                <span className={cn(
                  "font-mono",
                  (stats?.deadlocks || 0) > 0 ? "text-rose-400" : "text-emerald-400"
                )}>
                  {stats?.deadlocks || 0}
                </span>
              </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {sortedTables.slice(0, 8).map((table) => {
              const percentage = tables?.total_size_bytes 
                ? (table.size_bytes / tables.total_size_bytes) * 100 
                : 0
            ...
            {sortedTables.length > 8 && (
              <p className="text-xs text-muted-foreground text-center pt-2">
                + {sortedTables.length - 8} more tables
              </p>
            )}
          </div>
        </CardContent>
