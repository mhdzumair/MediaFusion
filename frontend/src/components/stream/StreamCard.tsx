...
export function StreamCard({ stream, onClick, showActions = true, showModeratorActions = true, onDeleted, mediaType = 'movie', isLastPlayed = false }: StreamCardProps) {
  ...
  const handleSaveAnnotation = useCallback(async (editedFiles: EditedFileLink[]) => {
    if (!stream.id) return
