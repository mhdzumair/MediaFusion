 ...
 */
export function ContentLikesBadge({ 
  mediaId,
  className 
}: { 
  mediaId: number
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <Badge 
            variant="outline" 
            className={cn(
              'text-xs gap-1 border-rose-500/50 text-rose-500',
              className
            )}
          >
        ...
        <TooltipContent>
          <p>{likes_count} {likes_count === 1 ? 'person likes' : 'people like'} this</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  )
