...
DROP INDEX IF EXISTS ix_stream_updated_at;
DROP INDEX IF EXISTS ix_media_updated_at;

CREATE INDEX IF NOT EXISTS idx_media_lower_title
    ON public.media (lower(title), type);

CREATE INDEX IF NOT EXISTS idx_media_blocked_by_user
    ON public.media (blocked_by_user_id)
    WHERE blocked_by_user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_media_last_refreshed_by_user
    ON public.media (last_refreshed_by_user_id)
    WHERE last_refreshed_by_user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_media_last_scraped_by_user
    ON public.media (last_scraped_by_user_id)
    WHERE last_scraped_by_user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_media_migrated_by_user
    ON public.media (migrated_by_user_id)
    WHERE migrated_by_user_id IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_episode_created_by_user
