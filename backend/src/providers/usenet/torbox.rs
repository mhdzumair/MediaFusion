...
pub async fn get_url(
    ...
    let list = mylist(http, token).await?;
    ...
    let usenet_info = match find_item(&list, name, nzb_guid) {
        ...
        None => {
            ...
            submit_nzb(http, token, submission_url, fallback_url, name, nzb_guid).await?
        }
    };
    ...
    let resp: Value = http
        .get(format!("{BASE}/usenet/requestdl"))
        .query(&[
...
async fn submit_nzb(
    ...
    if !url_to_try.is_empty() {
        let add = add_by_url(http, token, url_to_try).await?;
        let success = add
            .get("success")
        ...
        if success {
            ...
            return wait_for_item(http, token, name, nzb_guid).await;
        }
    ...
    let add2 = add_by_file(http, token, name, nzb_bytes).await?;
    let success2 = add2
        .get("success")
        .and_then(|v| v.as_bool())
        .unwrap_or(false);
    ...
    wait_for_item(http, token, name, nzb_guid).await
}
...
async fn add_by_url(
    ...
    http.post(format!("{BASE}/usenet/createusenetdownload"))
        .bearer_auth(token)
        .form(&[("link", nzb_url)])
        .send()
        .await?
...
async fn mylist(http: &reqwest::Client, token: &str) -> Result<Vec<Value>, ProviderError> {
    let resp: Value = http
        .get(format!("{BASE}/usenet/mylist"))
