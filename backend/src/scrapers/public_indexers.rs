...
pub fn extract_row_data_pub(doc: &Html, indexer: &IndexerDef) -> Vec<RowData> {
    extract_row_data(doc, indexer)
}
...
/// Public wrapper for size parsing — used by registry_crawl.
pub fn parse_size_bytes_pub(s: &str) -> Option<i64> {
    parse_size_bytes(s)
}
