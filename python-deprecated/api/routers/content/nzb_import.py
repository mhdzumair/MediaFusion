...
from reference.routers.content.contributions import award_import_approval_points
from reference.routers.content.import_title_validation import resolve_and_validate_import_title
from reference.routers.content.upload_guard import enforce_upload_permissions
from reference.routers.content.torrent_import import fetch_and_create_media_from_external
from reference.routers.user.auth import require_auth
...
async def process_nzb_import(
    ...
    existing_usenet = existing.first()
    if existing_usenet:
        if is_public:
            existing_stream = await session.get(Stream, existing_usenet.stream_id)
            if existing_stream and not existing_stream.is_public:
                ...
                return {
                    ...
                    "message": "Existing NZB stream published",
                }
...
async def import_nzb_file(
    ...
    try:
        ...
        if should_auto_approve and import_result and import_result.get("status") == "success":
            ...
            return NZBImportResponse(
                status="success",
                message="NZB file submitted for review and saved privately for your account.",
                import_id=contribution.id,
                details={
                    "nzb_guid": nzb_guid,
                    "title": contribution_data.get("title"),
...
async def import_nzb_url(
    ...
    try:
        ...
        if should_auto_approve and import_result and import_result.get("status") == "success":
            ...
            return NZBImportResponse(
                status="success",
                message="NZB URL submitted for review and saved privately for your account.",
                import_id=contribution.id,
                details={
                    "nzb_guid": nzb_guid,
                    "title": contribution_data.get("title"),
