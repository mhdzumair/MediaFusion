...
from reference.services.sync.manager import IntegrationManager
...
from workers.providers.cache_helpers import store_cached_info_hashes
...
async def fetch_stream_or_404(info_hash: str) -> tuple[TorrentStreamData, bool]:
    ...
    async def _load_from_primary() -> TorrentStreamData | None:
        async with get_async_session_context() as session:
            torrent_stream = await crud.get_stream_by_info_hash(session, info_hash, load_relations=True)
            if torrent_stream:
                return TorrentStreamData.from_db(torrent_stream)
    ...
    if settings.postgres_read_uri:
        stream_data = await run_db_read_with_primary_fallback(
    ...
    try:
        async with get_async_session_context() as session:
            ...
            if user_id:
                ...
                # Anonymous user — atomic increment; no SELECT needed, avoids hot-row ORM RMW
                from sqlalchemy import text as _text  # noqa: PLC0415

                await session.execute(
                    _text("UPDATE stream SET playback_count = COALESCE(playback_count, 0) + 1 WHERE id = :id"),
                    {"id": stream_id},
                )
                await session.commit()
