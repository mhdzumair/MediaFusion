...
async def get_all_rss_feeds(
    ...
    """Get all RSS feeds"""
    feeds = await crud.list_rss_feeds(session)
...
async def get_rss_feed(
    ...
    existing_feed = await crud.get_rss_feed(session, feed_id)
...
async def create_rss_feed(
    ...
    try:
        ...
        existing_feed = await crud.get_rss_feed_by_url(session, feed.url)
        if existing_feed:
            raise HTTPException(status_code=400, detail=f"RSS feed with URL {feed.url} already exists")
        ...
        new_feed = await crud.create_rss_feed(session, feed_data)
...
async def update_rss_feed_endpoint(
    ...
    updated_feed = await crud.update_rss_feed(session, feed_id, update_data)
...
async def delete_rss_feed(
    ...
    await crud.delete_rss_feed(session, feed_id)
    return {"detail": f"RSS feed {feed_id} deleted successfully"}


@router.post("/feeds/bulk-import")
async def bulk_import_rss_feeds(import_data: RSSFeedBulkImport, session: AsyncSession = Depends(get_async_session)):
    ...
    try:
        existing_feeds = await crud.list_rss_feeds(session)
        ...
        for feed_data in import_data.feeds:
            ...
            new_feed = await crud.create_rss_feed(session, feed_dict)
...
async def activate_deactivate_feeds(
    ...
    try:
        updated_count = await crud.bulk_update_rss_feed_status(session, feed_ids, activate)
