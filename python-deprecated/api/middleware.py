...
from utils.prometheus_metrics import HTTP_REQUESTS_IN_FLIGHT, record_http_metrics
...
class RequestIdFilter(logging.Filter):
    """Inject current request_id into every log record so lines are correlatable."""
...
class RequestIdMiddleware(BaseHTTPMiddleware):
    """Assign a unique request ID to every request, available via X-Request-Id header
    and the REQUEST_ID_VAR contextvar (injected into log records by RequestIdFilter)."""
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def check_rate_limit_with_redis(key: str, limit: int, window: int) -> bool:
        try:
            # transaction=False: INCR+EXPIRE don't need MULTI/EXEC atomicity; skipping
            # the round-trip saves ~0.5ms per request at high concurrency.
            results = await REDIS_ASYNC_CLIENT.pipeline(transaction=False).incr(key).expire(key, window).execute()
...
class TimingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Record Prometheus HTTP metrics (synchronous, very cheap)
        try:
            route = request.scope.get("route")
            route_template = route.path if route and hasattr(route, "path") else request.url.path
            record_http_metrics(request.method, route_template, response.status_code, process_time)
        except Exception:
            pass
...
# ---------------------------------------------------------------------------
# Profiler middleware (dev/staging only)
# ---------------------------------------------------------------------------

_PROFILE_PARAM = "_profile"


class ProfilerMiddleware(BaseHTTPMiddleware):
    """Wrap a single request with pyinstrument and return a flamegraph HTML.

    Activated by adding ``?_profile=1`` to any request, AND providing the
    admin API password in ``X-API-Key`` header (or the api_password query
    param used elsewhere). Requires ``ENABLE_PROFILER=true`` in settings.

    The HTML flamegraph is returned directly instead of the normal response
    so profiling never reaches the client cache or downstream. Use in
    dev/staging only — enabling in production exposes internal call stacks.
    """

    async def dispatch(self, request: Request, call_next):
        if request.query_params.get(_PROFILE_PARAM) != "1":
            return await call_next(request)

        # Require admin API key so arbitrary users can't trigger profiling.
        api_key = request.headers.get("x-api-key") or request.query_params.get("api_password", "")
        if api_key != settings.api_password:
            return JSONResponse(
                content={"detail": "Profiling requires admin API key via X-API-Key header."},
                status_code=403,
            )
        ...
        try:
            ...
            return JSONResponse(
                content={"detail": "pyinstrument not installed. Run: pip install pyinstrument"},
                status_code=501,
            )
        ...
        html = profiler.output_html(timeline=False)
        return Response(
            content=html,
