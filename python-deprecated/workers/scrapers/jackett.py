...
from workers.scrapers.base_scraper import IndexerBaseScraper
...
class JackettScraper(IndexerBaseScraper):
    ...
    async def parse_indexer_data(self, indexer_data: dict, catalog_type: str, parsed_data: dict) -> dict | None:
        ...
        try:
            ...
            self.logger.warning(
                "Skipping Jackett result due to HTTP status error while downloading torrent metadata (%s): %s",
                status_code,
                download_url,
            )
            return None
            ...
            self.logger.warning(
                "Skipping Jackett result due to request error while downloading torrent metadata: %s",
                error,
            )
            return None
