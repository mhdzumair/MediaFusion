...
_SCRAPY_SETTINGS_MODULE = "workers.scrapy.settings"
...
def run_spider(spider_name: str, *args, **kwargs):
    ...
    sigkill_max_retries = int(os.getenv("SCRAPY_PROCESS_SIGKILL_MAX_RETRIES", "2"))
    sigkill_retry_delay = int(os.getenv("SCRAPY_PROCESS_SIGKILL_RETRY_DELAY_SECONDS", "30"))

    for attempt in range(sigkill_max_retries + 1):
        logger.info(
            "Starting spider %s in subprocess (timeout=%ss, start_method=%s, attempt=%d/%d)",
            spider_name,
            timeout_seconds,
            _PROCESS_CONTEXT.get_start_method(),
            attempt + 1,
            sigkill_max_retries + 1,
        )
        p = None
        try:
            ...
            if p.exitcode == -9:
                if attempt < sigkill_max_retries:
                    logger.warning(
                        "Spider %s was killed by SIGKILL (likely OOM, attempt %d/%d). Retrying in %ds...",
                        spider_name,
                        attempt + 1,
                        sigkill_max_retries + 1,
                        sigkill_retry_delay,
                    )
                    time.sleep(sigkill_retry_delay)
                    continue
                raise RuntimeError(
                    f"Spider '{spider_name}' was killed by SIGKILL (likely OOM)"
                    f" after {sigkill_max_retries + 1} attempt(s)."
                )
