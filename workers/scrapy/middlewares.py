...
from workers.scrapy.scrapling_adapter import solve_protected_page
...
class ScraplingAntiBotMiddleware:
    ...
    async def process_request(self, request):
        ...
        if request.meta.get("force_scrapling_solve"):
            ...
            # If we already have valid CF cookies for this domain, try a plain
            # HTTP fetch first — the JS tokens we need are in static HTML, so
            # a new browser session is not required for every detail page.
            domain = urlparse(request.url).netloc
            cached = self.solved_domains.get(domain)
            if cached:
                last_solved_time, solution = cached
                if reactor.seconds() - last_solved_time < self.cache_duration:
                    resp = await self._fetch_with_cached_solution(request, solution)
                    if resp is not None:
                        return resp
                    # 403/503 — cached cookies are stale, force a fresh browser solve
                    self.solved_domains.pop(domain, None)
    ...
    async def _fetch_with_cached_solution(self, request, solution) -> TextResponse | None:
        """Fetch request.url with previously obtained CF cookies via plain HTTP.

        Returns a TextResponse on success, or None if the server rejects the
        cookies (403/503) so the caller can fall back to a full browser solve.
        """
        ...
        proxy_url = self.scraper_options.get("proxy_url")
        spider = self.crawler.spider
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(request.url, cookies=cookies, headers=headers)
        except Exception as exc:
            spider.logger.debug("Cached-cookie HTTP fetch failed for %s: %s", request.url, exc)
            return None

        if response.status_code in (403, 503):
            spider.logger.debug(
                "Cached CF cookies rejected (status=%s) for %s — will re-solve",
                response.status_code,
                request.url,
            )
            return None

        spider.logger.debug("Cached-cookie HTTP fetch succeeded for %s", request.url)
        if user_agent:
            request.meta["scrapling_user_agent"] = user_agent
        request.meta["scrapling_solved_cookies"] = dict(cookies)
        request.meta["anti_bot_solved"] = True
        return TextResponse(
