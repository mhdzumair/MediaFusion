...
async def _load_candidates(args: argparse.Namespace) -> tuple[list[BackfillPlan], dict[str, int]]:
    stats = {
        "scanned": 0,
        "parse_empty": 0,
...
async def _apply_plans(plans: list[BackfillPlan]) -> dict[str, int]:
    stats = {
        "updated_streams": 0,
        "failed_streams": 0,
        "scalar_updates": 0,
    ...
    async with get_async_session_context() as session:
        for plan in plans:
            try:
                query = (
                    select(Stream)
                    .where(Stream.id == plan.stream_id)
                    .options(
                        selectinload(Stream.languages),
                        selectinload(Stream.audio_formats),
                        selectinload(Stream.channels),
                ...
                stream = (await session.exec(query)).first()
                if not stream:
                    continue
...
async def run(args: argparse.Namespace) -> None:
    plans, discovery_stats = await _load_candidates(args)
    logger.info("Discovery stats: %s", discovery_stats)
    logger.info("Planned updates: %s streams", len(plans))

    _log_preview(plans, args.preview)

    if not args.apply:
        logger.info("Dry-run complete. Re-run with --apply to persist changes.")
        return

    apply_stats = await _apply_plans(plans)
