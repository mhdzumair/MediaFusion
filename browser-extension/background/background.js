...
class MediaFusionAPI {
  ...
  async uploadTorrent(torrentData) {
    ...
    if (torrentData.catalogs) {
      formData.append('catalogs', Array.isArray(torrentData.catalogs) ? torrentData.catalogs.join(',') : torrentData.catalogs);
    }
...
const handleMessage = async (message, sender, sendResponse) => {
  try {
    switch (message.action) {
      ...
      case 'analyzeTorrent':
        const analysisResult = await mediaFusionAPI.analyzeTorrent(message.data);
      ...
      case 'quickUpload':
        const quickResult = await mediaFusionAPI.quickUpload(message.data.magnetLink, message.data.options);
