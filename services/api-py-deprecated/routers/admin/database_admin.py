...
@router.get("/slow-queries", tags=["Admin Database Management"])
async def get_slow_queries(
    limit: int = Query(20, ge=1, le=100, description="Number of top queries to return"),
    min_calls: int = Query(5, ge=1, description="Minimum call count to include"),
    order_by: str = Query(
        "total_exec_time",
        description="Sort column: total_exec_time, mean_exec_time, calls, rows",
    ),
    _admin: User = Depends(require_role(UserRole.ADMIN)),
    session: AsyncSession = Depends(get_async_session),
):
    ...
    Requires ``pg_stat_statements`` extension (enabled in deployment configs).
    Queries are ordered by total execution time by default so the biggest
    overall contributors appear first, not just the slowest one-offs.

    Reset accumulated stats with ``POST /api/v1/admin/db/slow-queries/reset``.
    """
    valid_order = {"total_exec_time", "mean_exec_time", "calls", "rows"}
    if order_by not in valid_order:
        raise HTTPException(status_code=400, detail=f"order_by must be one of: {', '.join(sorted(valid_order))}")

    # Check extension availability first
    ext_check = await session.execute(text("SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements'"))
    if not ext_check.scalar():
        raise HTTPException(
            ...
            detail=(
                ...
                "and run 'CREATE EXTENSION IF NOT EXISTS pg_stat_statements;'."
            ),
        )
    ...
    result = await session.execute(
        text(f"""
            ...
            LIMIT :limit
        """),
        {"min_calls": min_calls, "limit": limit},
    )
...
@router.post("/slow-queries/reset", tags=["Admin Database Management"])
async def reset_slow_queries(
    _admin: User = Depends(require_role(UserRole.ADMIN)),
    session: AsyncSession = Depends(get_async_session),
):
    """Reset pg_stat_statements accumulated stats (clears all query history)."""
    ext_check = await session.execute(text("SELECT 1 FROM pg_extension WHERE extname = 'pg_stat_statements'"))
    if not ext_check.scalar():
        raise HTTPException(status_code=503, detail="pg_stat_statements extension is not installed.")
    await session.execute(text("SELECT pg_stat_statements_reset()"))
    return {"status": "ok", "message": "pg_stat_statements stats reset successfully."}
