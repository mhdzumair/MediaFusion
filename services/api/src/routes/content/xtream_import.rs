/// Xtream Codes API import endpoints (proxy to Python).
///
...
async fn proxy(
    ...
    let base = match &state.config.python_proxy_url {
        ...
        None => {
            return (
                ...
                Json(json!({"detail": "Xtream import service not available in this deployment"})),
            )
                .into_response();
        }
    };
    ...
    match req.send().await {
        ...
        Err(e) => {
            tracing::error!("xtream_import proxy error: {e}");
            (
