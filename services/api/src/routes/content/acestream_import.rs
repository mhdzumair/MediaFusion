/// AceStream hash import endpoints (proxy to Python for full contribution logic).
///
...
async fn proxy(
    ...
    let base = match &state.config.python_proxy_url {
        ...
        None => {
            return (
                ...
                Json(json!({"detail": "AceStream import service not available in this deployment"})),
            )
                .into_response();
        }
    };
    ...
    match req.send().await {
        ...
        Err(e) => {
            tracing::error!("acestream_import proxy error: {e}");
            (
...
pub async fn analyze_acestream(
    ...
    let body = axum::body::to_bytes(req.into_body(), 1024 * 1024)
        ...
        .unwrap_or_default()
        .to_vec();
    proxy(&state, reqwest::Method::POST, "/api/v1/import/acestream/analyze", &q, &headers, body).await
}
...
pub async fn import_acestream(
    ...
    let body = axum::body::to_bytes(req.into_body(), 4 * 1024 * 1024)
        ...
        .unwrap_or_default()
        .to_vec();
    proxy(&state, reqwest::Method::POST, "/api/v1/import/acestream", &q, &headers, body).await
}
